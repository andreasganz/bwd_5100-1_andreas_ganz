...
<head>
    <meta charset="utf-8">
    <title>Das Admin-Edit-Form</title>
...
<!-- Back-to-Dashboard-Button -->
        <a href="cms_dashboard.php"><img src="../pictures/cms/back-button_round_grey.svg" alt="Back-to-Dashboard-Button fürs CMS" class="dashboard_overview"><span class="button_description back">Dashboard</span></a>
              ...
              // Der "Löschen"-Befehl wird initiert und ausgeführt, wenn 'delete' aufgerufen wird
              if (isset($_GET['delete'])) {
                  // ID wird definiert, damit nur der gewünschte Eintrag gelöscht wird
                  $id = $_GET['delete'];

                  // Beim Löschen des Eintrags wird auch das Bild aus dem temporären Ordner gelöscht
                  $result = $db->query("SELECT * FROM data WHERE id=$id") or die($db->error);
                  if ($row = mysqli_fetch_array($result)) {
                      unlink("04_file-upload/images/".$row['image']);
                  }
              ...
              $result = $db->query("SELECT * FROM data ORDER BY date DESC") or die($db->error);
                  ...
                  while ($row = mysqli_fetch_array($result)): ?>
                      <div class="eintrag-auflistung">
                          ...
                          <!-- Headline -->
                          <?php echo "<h3>".$row['headline']."</h3>"; ?>

                          <!-- Copy -->
                          <?php echo "<p>".$row['copy']."</p>"; ?>

                          <!-- Edit-Button -->
                          <a href="05_file-edit/eintrag_bearbeiten_admin.php?edit=<?php echo $row['id']; ?>">
                          ...
                          <a href="admin.php?delete=<?php echo $row['id']; ?>">
                              <img class="delete-button" src="../pictures/cms/close-button_round_grey.svg"
                              alt="te-Button fürs CMS" name="delete">
                                  <span class="delete_description">Eintrag löschen</span>
                          </a>
                      </div>
              <?php endwhile; ?>
